...
// Handlers de cada página/vista de la SPA
// ============================================================
...
const PageHandlers = {
  ...
  // ==========================================
  async projects() {
    // Cargar catálogos en paralelo si no están cargados
        ...
        <div class="detail-grid">
            ...
            <div class="detail-section">
              ...
              <dl>
                <div class="detail-row">
                  <dt>Principal</dt>
                  <dd class="plain">${escHtml(p.owner_name ?? '—')} <span style="color:var(--text-muted)">${escHtml(p.owner_email ?? '')}</span></dd>
                </div>
                <div class="detail-row">
                  <dt>Secundario</dt>
                  <dd class="plain">${escHtml(p.sec_owner_name ?? '—')} <span style="color:var(--text-muted)">${escHtml(p.sec_owner_email ?? '')}</span></dd>
                </div>
              </dl>
            </div>
          </div>
          ...
          <div>
            <div class="detail-section" style="margin-bottom:16px">
              ...
              <dl>
                <div class="detail-row">
                  ...
                  <dd>${p.git_repo ? `<a href="${escHtml(p.git_repo)}" target="_blank">${escHtml(p.git_repo)}</a>` : '—'}</dd>
                </div>
                <div class="detail-row">
                  <dt>Servidor</dt>
                  <dd>${escHtml(p.server ?? '—')}</dd>
                </div>
                <div class="detail-row">
                  <dt>URL</dt>
            ...
            <div class="detail-section">
              <h4>Fechas</h4>
              <dl>
                <div class="detail-row"><dt>Creado</dt><dd>${formatDate(p.created_at)}</dd></div>
                <div class="detail-row"><dt>Modificado</dt><dd>${formatDate(p.updated_at)}</dd></div>
              </dl>
            </div>
          </div>
        </div>
  ...
  async technologies() {
    ...
    document.getElementById('tech-form').onsubmit = async (e) => {
      ...
      const data  = {
        name:  document.getElementById('tech-form-name').value.trim(),
  ...
  async users() {
    ...
    window.openUserModal = function(id = null) {
      ...
      const passNote = document.getElementById('user-pass-note');
      passNote.style.display = id ? 'block' : 'none';
...
window.openProjectModal = async function(id = null) {
  ...
  if (id) {
    try {
      ...
      document.getElementById('pf-git').value         = p.git_repo ?? '';
      document.getElementById('pf-server').value      = p.server ?? '';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('project-form').onsubmit = async (e) => {
    ...
    const selectedTechs = [...document.querySelectorAll('#pf-tech-picker .tech-tag.selected')]
                          .map(t => +t.dataset.id);
    const data = {
      ...
      git_repo:             document.getElementById('pf-git').value.trim(),
      server:               document.getElementById('pf-server').value.trim(),
...
window.deleteProject = function(id, name, goBack = false) {
  confirm(`¿Eliminar el proyecto "${name}"? Esta acción no se puede deshacer.`, async () => {
    try {
      ...
      if (goBack) App.navigate('projects');
